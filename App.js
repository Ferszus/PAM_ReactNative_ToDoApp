...
export default function App() {
    ...
    const saveEditedTask = () => {
        setTasks(prevTasks => {
            const updatedTasks = prevTasks.map(t => (t.id === editTaskId ? {...t, text: task, deadline: deadline || t.deadline} : t))
            saveTasksToStorage(updatedTasks)
    ...
    const handleConfirm = date => {
        setDeadline(`${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`)
        hideDatePicker()
    }
    ...
    const showAboutModal = () => {
        setAboutModalVisible(true) // Show the About modal
    }

    const hideAboutModal = () => {
        setAboutModalVisible(false) // Hide the About modal
    }

    const sortTasksByDeadline = () => {
        setTasks(prevTasks => {
            const tasksWithDeadline = prevTasks.filter(task => task.deadline !== "Brak terminu")
            const tasksWithoutDeadline = prevTasks.filter(task => task.deadline === "Brak terminu")
    ...
    return (
        <View style={[styles.container, {backgroundColor: isDarkMode ? "#555" : "#f4f7f9"}]}>
                    ...
                    <View style={[styles.taskContainer, isDarkMode && {backgroundColor: "#ddd"}, isToday(item.deadline) && item.completed ? styles.completedTaskContainer : isToday(item.deadline) ? styles.todayTaskContainer : isPastDeadline(item.deadline) ? styles.pastDeadlineTaskContainer : item.completed ? styles.completedTaskContainer : null]}>
                        ...
                        <View style={styles.taskButtons}>
                            <TouchableOpacity onPress={() => toggleCompletion(item.id)}>
                                <FontAwesome name={item.completed ? "check-square-o" : "square-o"} size={30} color={item.completed ? "#28a745" : "#007bff"} style={styles.icon} />
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => editTask(item.id)}>
                                <FontAwesome name="pencil" size={24} color="#007bff" style={styles.icon} />
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => deleteTask(item.id)}>
                                <FontAwesome name="trash" size={24} color="#dc3545" style={styles.icon} />
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
            ...
            {/* About Modal */}
            <Modal visible={aboutModalVisible} animationType="fade" transparent={true} onRequestClose={hideAboutModal}>
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        <Text style={styles.modalTitle}>Dev Team!</Text>
                        <Text style={styles.modalText}> Hubert Parylak - nr.alb. 71519</Text>
                        <Text style={styles.modalText}> Kacper Gasiul - nr.alb. 81550</Text>
                        <Text style={styles.modalText}> Tomasz Zańko - nr.alb. 68356</Text>
                        <Text style={styles.modalText}> Łukasz Jarębski - nr.alb. 67530</Text>

                        <TouchableOpacity style={styles.modalCancelButton} onPress={hideAboutModal}>
                            <Text style={styles.modalCancelButtonText}>Zamknij</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
